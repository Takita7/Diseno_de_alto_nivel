                              ...
                              sc_core::sc_time& delay)
{
    ...
    // --- Adaptación para el prototipo virtual ---
    // En la EC anterior, el Bus propio entregaba direcciones ya relativas
    // (0x00, 0x04, ...). Aquí las transacciones llegan directo desde gem5
    // (vía Gem5SlaveTransactor), con la dirección física absoluta. Se
    // resta la base una sola vez para no tocar el resto de la lógica de
    // decodificación de registros (sin cambios).
    constexpr uint64_t REGS_BASE = 0x30000000;
    const uint64_t offset = addr - REGS_BASE;
    ...
    if (cmd == tlm::TLM_WRITE_COMMAND) {
        ...
        write_reg(static_cast<uint32_t>(offset), value);
    }
    else if (cmd == tlm::TLM_READ_COMMAND) {
        value = read_reg(static_cast<uint32_t>(offset));
