...
TEST(LLVMBackendTest, EmitCudaKernelSource) {
    EXPECT_TRUE(initializeLLVMBackend());

    fs::path temp_dir = fs::temp_directory_path();
    fs::path source_file = temp_dir / "riscv_gpgpu_test_kernel.cu";
    fs::path output_file = temp_dir / "riscv_gpgpu_test_kernel_cuda.elf";
